 ...
 * @version $Revision$
 */
public final class NodeScope extends Scope {
	public static NodeScope getNodeScope() throws IllegalThreadStateException {
        return (NodeScope) getScope(NodeScope.class);
    }
	
	public NodeScope() {
	    super("PrimaryNodeThread", AbstractNodeConfiguration.class);
	}

	public static INode getNodeReference() {
        return (INode) getNodeScope().getContainerReference();
    }
	
	public static IHandle getNodeHandle(Class handleClass) {
	    return getNodeReference().getHandle(handleClass);
	}
}
